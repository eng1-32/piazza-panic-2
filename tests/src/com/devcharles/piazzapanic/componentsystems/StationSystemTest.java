...
public class StationSystemTest {
  ...
  public void stationPickupTest() {
    ...
    assertEquals("As the patty is not burned and isn't finished cooking, it should not be removed",
        0,
        controllable.currentFood.size());
    ...
    system.stationPickup(grillComponent, controllable);
    assertEquals(
        "As the patty has now become a grilled patty, it should be added to the player's stack",
    ...
    assertNull("The grilled patty in the station component should be removed",
        grillComponent.food.get(0));

    system.stationPickup(chopComponent, controllable);
    assertEquals(
        "As the lettuce has become sliced lettuce, it should be added to the player's stack",
        FoodType.slicedLettuce, Mappers.food.get(controllable.currentFood.pop()).type);
    assertNull("The sliced lettuce in the station component should be removed",
    ...
    assertEquals("If an ingredient is burnt, it should be picked up", 1,
        controllable.currentFood.size());
  ...
  public void hasBurnedTest() {
    ...
    Entity cuttingStation = factory.createStation(1, StationType.cutting_board, new Vector2(0, 0),
        null,
        false);
