...
public class WorldContactListenerTest {
  ...
  public void begin_and_endContactTest() {
    ...
    for (Contact contact : contacts) {
      ...
      if (objAStation || objBStation) {
        ...
        assertSame("Checks that the station's cook is assigned",
            stationComponent.interactingCook, cook);
        Mappers.transform.get(cook).position.set(new Vector3(2, 2, 2));
        contactListener.endContact(contact);
        assertNull("Checks that the cook is unassigned as interacting cook",
            stationComponent.interactingCook);

      } else {
        contactListener.beginContact(contact);
        assertSame("Checks that the customer's interacting cook is assigned",
            customerComponent.interactingCook, cook2);
        Mappers.transform.get(cook2).position.set(new Vector3(1, 1, 1));
        contactListener.endContact(contact);
        assertNull("Checks that cook is unassigned", customerComponent.interactingCook);
      }
    }
  }
  ...
  public void stationInteractResolverTest() {
    ...
    for (Contact contact : contacts) {
      ...
      if (objAStation || objBStation) {
        Pair<StationComponent, Entity> station_and_cook = worldContactListener.stationInteractResolver(
            contact);
    ...
    for (Contact contact : contacts) {
      ...
      if (Mappers.customer.has(objA)) {
        if (Mappers.player.has(objB)) {
          ...
          Pair<Entity, Entity> customer_and_cook = contactListener.customerInteractResolver(
              contact);
          assertTrue("Asserts that pickUp is still set to True",
              Mappers.player.get(customer_and_cook.second).pickUp);
          assertFalse("Checks that putDown is set to false",
              Mappers.player.get(customer_and_cook.second).putDown);

        } else {
          assertNull("The function should return null otherwise",
              contactListener.customerInteractResolver(contact));
        }
